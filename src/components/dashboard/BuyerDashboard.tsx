...
export default function BuyerDashboard() {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    if (!pickupStation) {
      toast({ title: "Select a pickup station", variant: "destructive" });
      return;
    }
  ...
  const processOrder = async () => {
    ...
    for (const item of cart) {
      ...
      if (stockError) {
        ...
        // We don't return here because the order was already placed, 
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (pin.length < 4) {
      toast({ title: "Invalid PIN", description: "Please enter a valid Mobile Money PIN.", variant: "destructive" });
      return;
    }
    await processOrder();
  };
  ...
  return (
    <div>
            ...
            <TableBody>
              {orders.map(o => (
                <TableRow key={o.id}>
                  ...
                  <TableCell>{o.pickup_station || "—"}</TableCell>
        ...
        <DialogContent>
          ...
          <form onSubmit={handleCheckout} className="space-y-4">
            ...
            <div className="space-y-3">
              <Label className="text-base font-semibold">Pickup Station</Label>
              <Select value={pickupStation} onValueChange={setPickupStation}>
                <SelectTrigger>
                  <SelectValue placeholder="Select a nearby pickup station" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="Eso">Eso</SelectItem>
                  <SelectItem value="Mwanjari">Mwanjari</SelectItem>
                  <SelectItem value="Kabale Central Market">Kabale Central Market</SelectItem>
                  <SelectItem value="Muhanga Town">Muhanga Town</SelectItem>
                  <SelectItem value="Rushebeya Trading Center">Rushebeya Trading Center</SelectItem>
                  <SelectItem value="Bukinda Trading Center">Bukinda Trading Center</SelectItem>
                  <SelectItem value="Muko">Muko</SelectItem>
                </SelectContent>
              </Select>
            </div>
      ...
      {/* Mobile Money PIN Dialog (Simulation) */}
      <Dialog open={pinDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-xs">
          ...
          <div className="text-center mb-4 text-sm">
            <p>Enter your Mobile Money PIN to authorize <strong>UGX {cartTotal.toLocaleString()}</strong> for FarmConnect.</p>
          </div>
          <form onSubmit={handlePinSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full">Authorize Payment</Button>
          </form>
        </DialogContent>
      </Dialog>
