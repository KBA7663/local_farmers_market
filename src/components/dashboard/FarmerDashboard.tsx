...
export default function FarmerDashboard() {
    ...
    if (data) {
      const enriched = await Promise.all(data.map(async (o) => {
        const [prodRes, buyerRes] = await Promise.all([
          ...
          supabase.from("profiles").select("full_name, phone").eq("user_id", o.buyer_id).single(),
        ]);
        return { ...o, product_name: prodRes.data?.name, buyer_name: buyerRes.data?.full_name, buyer_phone: buyerRes.data?.phone };
      }));
      setOrders(enriched);
    }
  };
  ...
  const handleUpdateOrderStatus = async (orderId: string, status: Database["public"]["Enums"]["order_status"]) => {
    ...
    if (status === "confirmed") {
      // SMS notification removed by user request
    }
